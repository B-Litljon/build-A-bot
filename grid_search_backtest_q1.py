...
for profile_name, sl_mult, tp_mult in risk_profiles:
    ...
    if trades:
        ...
        print(f"  ✅ {len(trades)} trades | WR: {wr:.1%} | PF: {pf:.2f} | PnL: ${total:.2f}")
...
print("=" * 80)
print(f"{'Configuration':<20} {'Trades':>8} {'Win Rate':>10} {'P&F':>8} {'PnL':>12} {'Status':<15}")
print("-" * 80)

best_config = None
for r in results:
    status = "✅ PASS" if r["trades"] >= 50 and r["profit_factor"] > 1.2 else "❌ FAIL"
    print(f"{r['profile']:<20} {r['trades']:>8} {r['win_rate']:>9.1%} {r['profit_factor']:>8.2f} ${r['pnl']:>10.2f} {status:<15}")
...
if best_config:
    ...
    print(f"   SL: {(1-best_config['sl'])*100:.1f}% | TP: {(best_config['tp']-1)*100:.1f}%")
    ...
    print(f"   TP: {best_config['tp_hits']} | SL: {best_config['sl_hits']} | Timeout: {best_config['timeout_exits']}")
    print(f"\n   ✅ READY FOR PAPER TRADING!")
else:
    print("\n   ⚠️  No configuration met success criteria")
    print(f"\n   📊 Detailed Breakdown:")
    for r in results:
        print(f"      {r['profile']}: PF={r['profit_factor']:.2f}, Trades={r['trades']}")
    ...
    for r in results:
        if r["trades"] > 0:
            sl_pct = (1-r["sl"])*100
            tp_pct = (r["tp"]-1)*100
            print(f"      {r['profile']}: {r['sl_hits']} SL hits vs {r['tp_hits']} TP hits")
            if r["sl_hits"] > r["tp_hits"] * 2:
                print(f"         → Tight SL ({sl_pct:.1f}%) causing excessive stop-outs")
            if r["timeout_exits"] > r["trades"] * 0.3:
                print(f"         → High timeout rate ({r['timeout_exits']}/{r['trades']}), consider extending hold time")
